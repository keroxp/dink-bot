...
async function getLatestDenoVersion(): Promise<string> {
  const resp = await fetch("https://api.github.com/repos/denoland/deno/releases");
  if (resp.status === 200) {
...
async function updateModuleJson(denoVersion: string) {
  ...
  if (mod["https://deno.land/std"]) {
    console.log("Updating modules.json https://deno.land/std version to "+denoVersion)
    mod["https://deno.land/std"]["version"] = "@"+denoVersion;
    Deno.writeFile("modules.json", encoder.encode(JSON.stringify(mod, null, "  ")));
    console.log("Updated modules.json")
  }
}

async function runDink() {
  await exec(["deno", "run", "-A","-r", "https://denopkg.com/keroxp/dink/main.ts"]);
}
...
async function commitChanges(denoVersion: string) {
  await exec(["git", "config" ,"--local", "user.email", "kerokerokerop@gmail.com"]);
  await exec(["git", "config", "--local", "user.name", "keroxp-bot"]);
  await exec(["git", "checkout", "-b", `botbump-deno@${denoVersion}`]);
  await exec(["git", "add", "."]);
  await exec(["git", "commit", "-m", `"bump: deno@${denoVersion}", std@${denoVersion}`]);
